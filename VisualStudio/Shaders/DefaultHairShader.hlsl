...
float4 ps_main(GFSDK_Hair_PixelShaderInput input) : SV_Target
{
	...
	float4 r = float4(0.0, 0.0, 0.0, 1.0); //= mat.rootColor;

    if (GFSDK_Hair_VisualizeColor(g_hairConstantBuffer, mat, attr, r.rgb)) {
		return r;
     }

	// texture are "upside down" in Unity
	float3 textCoord = attr.texcoords;
	textCoord.y = 1.0-textCoord.y;
    
	// compute the hair color 
	float3 hairColor = GFSDK_Hair_SampleHairColorTex(g_hairConstantBuffer, mat, texSampler, g_rootHairColorTexture, g_tipHairColorTexture, textCoord);

	// do all lighting stuff
	...
	return r;
	//return float4(hairColor.r, hairColor.g, hairColor.b, 1.0); 
}
